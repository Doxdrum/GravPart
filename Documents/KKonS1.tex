...
\begin{center}
  \begin{tikzpicture}[thick,line cap=round,
    ...
    \foreach \y in {1,2,...,4}
    \node[anchor=west] at (2,\y) {${\small \times}2$};
...
\begin{center}
  \begin{tikzpicture}[yscale=.5,thick,line cap=round,
      % Styles
      axes/.style=,
      important line/.style={very thick},
      information text/.style={rounded corners,draw=blue!80!black,fill=blue!10,inner sep=1ex}]
    ...
    \draw (0,0) node[anchor=east] {$m_{\text{eff}} =0$} -- (2,0);
    \foreach \y/\m in {1/3,2/5,3/7}
    {
      \draw[important line] (0,{\y*(\y+1)}) node[anchor=east] {$m_{\text{eff}} =\frac{\pgfmathparse{\y*(\y+1)}\pgfmathresult}{R}$} -- (2,{\y*(\y+1)}) node[anchor=west]  {${\tiny \times}\m$};
    }
    \node at (1,14) {$\vdots$};

    \draw[xshift=3.5cm,yshift=6.5cm]
    node[right,text width=6cm,information text]
